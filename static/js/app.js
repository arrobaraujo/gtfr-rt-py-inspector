 ...
 *
 * Handles: map rendering, data polling, marker management,
 * popup generation, shape/stop overlays, and UI state.
 */
...
async function init() {
    await loadSettings();
    pollData();
    startPolling(30000); // Default: 30 seconds

    // Set initial language
    setLanguage(window.currentLang || "pt");

    // Event: toggle stop markers
    ...
    // Event: focus on selected vehicle
    ...
    // Event: speed unit toggle — refresh popups
    if (chkSpeedKmh) {
...
async function saveSettings() {
    try {
        ...
        const clamped = Math.max(2, Math.min(seconds, 300));
        startPolling(clamped * 1000);
...
async function pollData() {
    ...
    try {
        ...
        console.error("Error fetching RT data:", e);
    } finally {
...
function updateMap(vehicles) {
    ...
    const focusedId = shouldFocus
        ? currentSelectedVehicle.vehicle_id || currentSelectedVehicle.id
        : null;

    vehicles.forEach(v => {
        const vId = v.vehicle_id || v.id;

        // Skip non-focused vehicles when focus mode is on
...
function t(key) {
    const lang = translations[window.currentLang];
...
function createPopupContent(v) {
    ...
    const fare =
        v.fare_price !== null && v.fare_price !== undefined
            ? v.fare_price.toLocaleString("pt-BR", {
                  style: "currency",
                  currency: v.fare_currency || "BRL",
              })
            : t("na");
    const departure = v.start_time || t("na");

    // Direction: 0 = Ida/Outbound, 1 = Volta/Inbound
    let direction = t("na");
    if (v.direction_id === "0" || v.direction_id === 0) direction = "0 - " + t("dir_0");
    else if (v.direction_id === "1" || v.direction_id === 1) direction = "1 - " + t("dir_1");

    // Speed: apply m/s → km/h conversion unless checkbox says feed is already km/h
    let speed = v.speed || 0;
    if (!(chkSpeedKmh && chkSpeedKmh.checked)) speed *= 3.6;
    ...
    return `
        <div class="popup-content">
            ...
            <p><strong>${t("popup_depart")}:</strong> ${departure}</p>
...
async function handleMarkerClick(vehicle) {
    ...
    try {
        ...
        currentShapeLayer = L.geoJSON(shapeData, {
            filter: feature => {
                return feature.properties.type === "stop" ? showStops : true;
            },
            ...
            onEachFeature: (feature, layer) => {
                if (feature.properties.type === "stop") {
                    ...
                    layer.bindPopup(`
                        <div class="popup-content" style="font-size: 13px; line-height: 1.4;">
                            <h3 style="margin: 0 0 8px 0; color: var(--accent); font-size: 1.1rem;">${p.stop_name || t("na")}</h3>
                            <p><strong>${t("popup_desc")}:</strong> ${p.stop_desc || t("na")}</p>
                            <p><strong>${t("popup_code")}:</strong> ${p.stop_code || t("na")}</p>
                            <p><strong>${t("popup_platform")}:</strong> ${p.platform_code || t("na")}</p>
                            <p><strong>${t("popup_parent")}:</strong> ${p.parent_station || t("na")}</p>
                            <p><strong>${t("popup_type")}:</strong> ${p.location_type || t("na")}</p>
                        </div>
                    `);
                }
            },
            style: feature => {
                if (feature.properties.type === "shape") {
                    return { color: shapeColor, weight: 4, opacity: 0.8 };
                }
            },
        }).addTo(map);
    } catch (e) {
        console.error("Error loading trip shape:", e);
    }
}
